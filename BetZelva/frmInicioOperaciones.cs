...
namespace BetZelva
{
    public partial class frmInicioOperaciones : Form
    {
        ...
        private void frmInicioOperaciones_Load(object sender, EventArgs e)
        {
            ...
            if (pidUsuario == 0)
            {
                MyMessageBox.Show("No existe usuario", "Validar Usuario", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Dispose();
                return;
            }
        ...
        {
            var Msg = MyMessageBox.Show("Esta seguro de Realizar el Inicio de Operaciones?...", "Inicio de Operaciones", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (Msg == DialogResult.Yes)
            {
                ...
                if (Rpta == "OK")
                {
                    MyMessageBox.Show("El Inicio de Operaciones se Realizó Correctamente...", "Inicio de Operaciones", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    btnGrabar.Enabled = false;
                }
                else
                {
                    MyMessageBox.Show(Rpta, "Inicio de Operaciones", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
